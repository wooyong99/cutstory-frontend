...
import { fetchMenuDetail, updateMenu, fetchCategories } from '../../services/api';
...
export function MenuEditPage() {
  ...
  const { data: menuDetail, isLoading: isMenuLoading } = useQuery({
  ...
  useEffect(() => {
    ...
    setMinDuration(String(menuDetail.minDuration));
    setMaxDuration(String(menuDetail.maxDuration));
    setPrice(String(menuDetail.price));
    setMainImage(menuDetail.mainImage);
    setDetailImages(
      menuDetail.detailImages.length > 0
        ? menuDetail.detailImages.map((img) => img.imageUrl)
        : [''],
    );
    setOptions(
      menuDetail.options.map((opt) => ({
        name: opt.name,
  ...
  const mutation = useMutation({
    mutationFn: (data: Parameters<typeof updateMenu>[1]) => updateMenu(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'menus'] });
      queryClient.invalidateQueries({ queryKey: ['menuDetail', id] });
    ...
    return (
      <div>
        <div className="admin-page-header">
          <h1 className="admin-page-title">메뉴 수정</h1>
        </div>
  ...
  return (
    <div>
      <div className="admin-page-header">
        <h1 className="admin-page-title">메뉴 수정</h1>
        <p className="admin-page-description">메뉴 정보를 수정합니다.</p>
      </div>
